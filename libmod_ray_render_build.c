    ...
    for (int x = draw_x1; x <= draw_x2; x++) {
        ...
        for (int w = 0; w < sector->num_walls; w++) {
             ...
             if (fabsf(det) > 0.0001f) {
                 ...
                 if (s_chk >= -0.001f && s_chk <= 1.001f && t > 0.1f) { // Valid hit forward (with epsilon)
                     hit_count++;
                     if (t < t_near) {
                         t_near = t;
                         near_wall = wall;
        ...
        if (t_far > 0.1f) {
            ...
            // If even hits, we are outside (Entry found). If odd hits, we started inside (t_near is actually Exit).
                      ...
                      if (t_near * view_dist < g_zbuffer[pixel_idx]) {
                          ...
                          if (wall_tex) {
                              ...
                              if (tex_y == last_tex_y && g_engine.texture_quality == 0) {
                                  ...
                                  if (g_engine.texture_quality == 1) {
                                      pix = ray_sample_texture_bilinear(wall_tex, near_u, curr_v);
                                  } else {
                                      pix = gr_get_pixel(wall_tex, tex_x, tex_y);
                                  }
                                  last_tex_y = tex_y;
                                  cached_pixel = pix;
                              }
...
static void render_hole_stencil(GRAPH *dest, int sector_id, int min_x, int max_x) {
    ...
    // Note: This is a simplified approach. For proper visibility of nested rooms,
    // the parent sector's walls that are shared with children should be marked as portals
...
void render_sector(GRAPH *dest, int sector_id, int min_x, int max_x, int depth)
{
    ...
    if (sector->num_children > 0 && sector->child_sector_ids) {
        for (int i = 0; i < sector->num_children; i++) {
            ...
            if (!ray_aabb_visible(child)) {
                // g_engine.stats.culled_sectors++; // Optional stat
                continue; 
            }
