...
static inline int ray_sector_is_solid(RAY_Sector *sector) {
  ...
  // A sector is "solid" (rendered as a block) if:
  // 1. It has NO portals (it's isolated geometry, not a walkable room)
  // It can have children (nested sectors like building floors) and still be
  // solid.
  if (sector->num_portals == 0)
    return 1;
...
static inline int ray_sector_get_child(RAY_Sector *sector, int index) {
  if (!sector)
    return -1;
  if (index < 0 || index >= sector->num_children)
    return -1;
