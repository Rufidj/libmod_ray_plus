...
RAY_MD2_Model *ray_md2_load(const char *filename) {
  ...
  size_t dummy;
  dummy = fread(&model->header, sizeof(md2_header_t), 1, fp);
  (void)dummy;

  if (model->header.magic != MD2_MAGIC ||
      model->header.version != MD2_VERSION) {
    printf("RAY_MD2: Invalid MD2 file %s (Magic: %d, Version: %d)\n", filename,
           (int)model->header.magic, (int)model->header.version);
    free(model);
    fclose(fp);
    return NULL;
  }
  ...
  model->texCoords = (md2_texCoord_t *)malloc(sizeof(md2_texCoord_t) *
                                              model->header.numTexCoords);
  fseek(fp, model->header.offsetTexCoords, SEEK_SET);
  if (fread(model->texCoords, sizeof(md2_texCoord_t),
            model->header.numTexCoords,
            fp) != (size_t)model->header.numTexCoords) {
    printf("RAY_MD2: Warning - Error reading texCoords in %s\n", filename);
  }

  /* 3. Allocate and Read Triangles */
  model->triangles = (md2_triangle_t *)malloc(sizeof(md2_triangle_t) *
                                              model->header.numTriangles);
  fseek(fp, model->header.offsetTriangles, SEEK_SET);
  if (fread(model->triangles, sizeof(md2_triangle_t),
            model->header.numTriangles,
            fp) != (size_t)model->header.numTriangles) {
    printf("RAY_MD2: Warning - Error reading triangles in %s\n", filename);
  }

  /* 4. Allocate and Read Frames */
  model->frames =
      (md2_frame_t *)malloc(sizeof(md2_frame_t) * model->header.numFrames);
  ...
  for (int i = 0; i < model->header.numFrames; i++) {
     ...
     */
    if (fread(model->frames[i].scale, sizeof(float), 3, fp) != 3) {
      /* Skip or handle error */
    }
    if (fread(model->frames[i].translate, sizeof(float), 3, fp) != 3) {
      /* Skip or handle error */
    }
    if (fread(model->frames[i].name, sizeof(char), 16, fp) != 16) {
      /* Skip or handle error */
    }

    /* Allocate vertices for this frame */
    model->frames[i].vertices = (md2_vertex_t *)malloc(
        sizeof(md2_vertex_t) * model->header.numVertices);
    if (fread(model->frames[i].vertices, sizeof(md2_vertex_t),
              model->header.numVertices,
              fp) != (size_t)model->header.numVertices) {
      /* Error */
    }
  }
  ...
  printf("RAY_MD2: Loaded %s (Frames: %d, Polys: %d)\n", filename,
         (int)model->header.numFrames, (int)model->header.numTriangles);
  return model;
}
...
void ray_md2_interpolate_vertex(RAY_MD2_Model *model, int frame1, int frame2,
                                ...
                                md2_vec3_t *out) {
  if (!model || frame1 < 0 || frame1 >= model->header.numFrames || frame2 < 0 ||
      frame2 >= model->header.numFrames)
    return;
