...
int64_t libmod_ray_init(INSTANCE *my, int64_t *params) {
    ...
    g_engine.rayCount = g_engine.internalWidth / strip_width;  // Use internal width
    g_engine.viewDist = ray_screen_distance((float)g_engine.internalWidth, g_engine.fovRadians);
...
void ray_calculate_all_sector_bounds() {
    for (int i = 0; i < g_engine.num_sectors; i++) {
        ...
        if (sector->num_walls == 0) {
            sector->min_x = 0; sector->min_y = 0;
            sector->max_x = 0; sector->max_y = 0;
            continue;
        }
        ...
        for (int w = 0; w < sector->num_walls; w++) {
            ...
            // Checks for X1, Y1
            if (wall->x1 < sector->min_x) sector->min_x = wall->x1;
            if (wall->x1 > sector->max_x) sector->max_x = wall->x1;
            if (wall->y1 < sector->min_y) sector->min_y = wall->y1;
            if (wall->y1 > sector->max_y) sector->max_y = wall->y1;
            
            // Checks for X2, Y2 (Redundant if walls are connected, but safer)
            if (wall->x2 < sector->min_x) sector->min_x = wall->x2;
            if (wall->x2 > sector->max_x) sector->max_x = wall->x2;
            if (wall->y2 < sector->min_y) sector->min_y = wall->y2;
            if (wall->y2 > sector->max_y) sector->max_y = wall->y2; 
        }
    }
}
...
int64_t libmod_ray_move_forward(INSTANCE *my, int64_t *params) {
    ...
    if (!ray_check_collision(&g_engine, g_engine.camera.x, g_engine.camera.y, g_engine.camera.z, newX, newY)) {
        ...
        RAY_Sector *sector = ray_find_sector_at_position(&g_engine, newX, newY, g_engine.camera.z);
        if (sector) {
            g_engine.camera.current_sector_id = sector->sector_id;
