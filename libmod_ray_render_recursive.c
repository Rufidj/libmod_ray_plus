...
static int project_z_to_screen_y(float world_z, float distance, float camera_z) {
    if (distance < 0.1f) return g_engine.displayHeight / 2;
    
    float dz = world_z - camera_z;
    float screen_y = (g_engine.displayHeight / 2.0f) - (dz * g_engine.viewDist / distance);
    
    return (int)screen_y;
}

/* Recursive sector rendering for a single column */
void render_sector_column_recursive(
    GRAPH *dest,
    int screen_x,
    float ray_angle,
    int sector_id,
    ...
    int depth)
{
    ...
    if (clip_top >= clip_bottom) return;  /* No visible space */
    ...
    ray_cast_ray(sector_id, g_engine.camera.x, g_engine.camera.y, ray_angle, hits, &num_hits);
    
    if (num_hits == 0) {
        /* No walls hit - render floor/ceiling to horizon */
        ray_draw_floor_ceiling(dest, screen_x, ray_angle, sector_id, 
                              0.0f, g_engine.viewDist * 4.0f, z_buffer, 
    ...
    for (int h = 0; h < num_hits; h++) {
        ...
        if (hit->distance > current_dist + 0.1f) {
            ray_draw_floor_ceiling(dest, screen_x, ray_angle, sector_id,
                                  current_dist, hit->distance, z_buffer,
                                  &local_clip_top, &local_clip_bottom);
        }
        
        /* Render the wall */
        ray_draw_wall_strip(dest, hit, screen_x, &local_clip_top, &local_clip_bottom);
        ...
        if (wall_bottom < local_clip_top) local_clip_top = wall_bottom;
        if (wall_top > local_clip_bottom) local_clip_bottom = wall_top;
        ...
        for (int c = 0; c < sector->num_children; c++) {
            ...
            if (ray_point_in_polygon(hit_x, hit_y, child->vertices, child->num_vertices)) {
                /* Calculate child sector's ceiling/floor screen positions */
                int child_ceiling_y = project_z_to_screen_y(child->ceiling_z, hit->distance, g_engine.camera.z);
                int child_floor_y = project_z_to_screen_y(child->floor_z, hit->distance, g_engine.camera.z);
                
                /* Intersect with current clip bounds */
                int new_clip_top = (child_ceiling_y > local_clip_top) ? child_ceiling_y : local_clip_top;
                int new_clip_bottom = (child_floor_y < local_clip_bottom) ? child_floor_y : local_clip_bottom;
                
                /* Recurse into child sector */
                if (new_clip_top < new_clip_bottom) {
                    render_sector_column_recursive(dest, screen_x, ray_angle, child_id,
                                                  new_clip_top, new_clip_bottom, z_buffer, depth + 1);
                }
            }
        }
