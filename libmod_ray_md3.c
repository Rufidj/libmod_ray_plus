...
RAY_MD3_Model *ray_md3_load(const char *filename) {
    ...
    if (model->header.magic != MD3_MAGIC || model->header.version != MD3_VERSION) {
    ...
    if (model->header.numFrames > 0) {
        model->frames = (md3_frame_t *)malloc(sizeof(md3_frame_t) * model->header.numFrames);
        fseek(fp, model->header.offsetFrames, SEEK_SET);
        if (fread(model->frames, sizeof(md3_frame_t), model->header.numFrames, fp) != (size_t)model->header.numFrames) {
    ...
    if (model->header.numTags > 0) {
        model->tags = (md3_tag_t *)malloc(sizeof(md3_tag_t) * model->header.numTags * model->header.numFrames);
        fseek(fp, model->header.offsetTags, SEEK_SET);
        if (fread(model->tags, sizeof(md3_tag_t), model->header.numTags * model->header.numFrames, fp) != (size_t)(model->header.numTags * model->header.numFrames)) {
    ...
    if (model->header.numSurfaces > 0) {
        model->surfaces = (RAY_MD3_Surface *)malloc(sizeof(RAY_MD3_Surface) * model->header.numSurfaces);
        memset(model->surfaces, 0, sizeof(RAY_MD3_Surface) * model->header.numSurfaces);
        ...
        for (int i = 0; i < model->header.numSurfaces; i++) {
            ...
            if (surf->header.numShaders > 0) {
                surf->shaders = (md3_shader_t *)malloc(sizeof(md3_shader_t) * surf->header.numShaders);
                fseek(fp, surfaceStart + surf->header.offsetShaders, SEEK_SET);
                fread(surf->shaders, sizeof(md3_shader_t), surf->header.numShaders, fp);
            }
            
            /* Read Triangles */
            if (surf->header.numTriangles > 0) {
                surf->triangles = (md3_triangle_t *)malloc(sizeof(md3_triangle_t) * surf->header.numTriangles);
                fseek(fp, surfaceStart + surf->header.offsetTriangles, SEEK_SET);
                fread(surf->triangles, sizeof(md3_triangle_t), surf->header.numTriangles, fp);
            }
            
            /* Read TexCoords */
            if (surf->header.numVerts > 0) {
                surf->texCoords = (md3_texCoord_t *)malloc(sizeof(md3_texCoord_t) * surf->header.numVerts);
                fseek(fp, surfaceStart + surf->header.offsetTexCoords, SEEK_SET);
                fread(surf->texCoords, sizeof(md3_texCoord_t), surf->header.numVerts, fp);
            }
            
            /* Read Vertices (numVerts * numFrames) */
            if (surf->header.numVerts > 0) {
                int totalVerts = surf->header.numVerts * surf->header.numFrames;
                surf->vertices = (md3_vertex_t *)malloc(sizeof(md3_vertex_t) * totalVerts);
                fseek(fp, surfaceStart + surf->header.offsetXyzNormals, SEEK_SET);
                fread(surf->vertices, sizeof(md3_vertex_t), totalVerts, fp);
            }
    ...
    fclose(fp);
    printf("RAY_MD3: Loaded %s (Surfaces: %d, Frames: %d)\n", filename, model->header.numSurfaces, model->header.numFrames);
    return model;
}
...
void ray_md3_free(RAY_MD3_Model *model) {
    ...
    if (model->frames) free(model->frames);
    if (model->tags) free(model->tags);
    
    if (model->surfaces) {
        for (int i = 0; i < model->header.numSurfaces; i++) {
            RAY_MD3_Surface *surf = &model->surfaces[i];
            if (surf->shaders) free(surf->shaders);
            if (surf->triangles) free(surf->triangles);
            if (surf->texCoords) free(surf->texCoords);
            if (surf->vertices) free(surf->vertices);
        }
        free(model->surfaces);
    }
