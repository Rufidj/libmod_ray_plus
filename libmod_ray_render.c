...
/* Vertical Linear Sampling - Only interpolates vertically to avoid horizontal artifacts */
                          ...
                          int *ceiling_clip, int *floor_clip)
{
    ...
    int wall_bottom = g_engine.displayHeight / 2 + (int)g_engine.camera.pitch + (int)player_screen_z;
                             ...
                             float *z_buffer, int *ceiling_clip, int *floor_clip)
{
    ...
    if (is_solid_child) {
        ...
        if (g_engine.camera.z < sector->floor_z) {
            ...
            top_tex = sector->floor_texture_id; // Use floor texture for bottom face? Or ceiling?
                                            // Convention: Floor texture on floor surface. 
                                            // The bottom of a crate is its floor.
                                            // So use floor_texture.
        }
        ...
        if (g_engine.camera.z > sector->ceiling_z) {
            ...
            bottom_tex = sector->ceiling_texture_id; // Use ceiling texture for top face.
        }
    }
...
void ray_draw_sprites(GRAPH *dest, float *z_buffer)
{
    ...
    for (int i = 0; i < g_engine.num_sprites; i++) {
        ...
        // Skip FOV check for MD2 models as they manage their own visibility/clipping
        if (!sprite->model && fabsf(angle_diff) > g_engine.fovRadians / 2.0f + 0.5f) continue;
    ...
    for (int strip = 0; strip < g_engine.rayCount; strip++) {
        ...
        for (int h = 0; h < num_hits; h++) {
            ...
            if (hits[h].wall) {
                ...
                /* Usually sector_id == index if sequentially created, but safest is lookup or direct check if we have pointer. */
                /* hits[h].wall->sector? No. */
                /* But hits[h].sector_id is available. */
                
                RAY_Sector *sec = NULL;
                // Optimization: try direct index if it matches
                if (hits[h].sector_id >= 0 && hits[h].sector_id < g_engine.num_sectors && 
                    g_engine.sectors[hits[h].sector_id].sector_id == hits[h].sector_id) {
        ...
        for (int h = 0; h < num_hits; h++) {
            ...
            if (hit->wall->portal_id >= 0) {
                ...
                /* The floor/ceiling behind it will be clipped by Z-buffer where the wall exists */
        ...
        for (int h = 0; h < num_hits; h++) {
            if (hits[h].is_child_sector) {
                 ...
                 /* The 'ray_draw_floor_ceiling' function has been updated to handle Solid Child logic */
                 ray_draw_floor_ceiling(dest, screen_x, ray_angle,
                                       hits[h].sector_id, 
